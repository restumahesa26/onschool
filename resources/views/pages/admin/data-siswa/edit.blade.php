...
<div class="section-header">
    <h1>Edit Data Siswa {{ $item->nama }}</h1>
</div>
<div class="card">
    <div class="card-body">
        <form action="{{ route('data-siswa.update', $item->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="kelas" id="kelas" class="form-control @error('kelas') is-invalid @enderror" placeholder="Masukkan Kelas" value="{{ $item->kelas }}">
                @error('kelas')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
